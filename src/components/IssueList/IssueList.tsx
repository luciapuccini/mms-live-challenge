...
export function IssueList(): JSX.Element {
  const { data, loading, error } =
    useQuery<GetReactIssuesData>(GET_REACT_ISSUES);

  if (loading)
    return (
      <div data-testid="loading-state" className={styles.loading}>
        Loading issues...
      </div>
    );
  if (error)
    return (
      <div data-testid="error-state" className={styles.error}>
        Error: {error.message}
      </div>
    );
      ...
      {edges.map((edge) => {
        ...
        return (
          <Card key={issue.id} className={cardStyles.itemCard}>
            <div
              className={cardStyles.itemInner}
              role="article"
              aria-labelledby={`issue-${issue.id}`}
            >
              <div className={cardStyles.meta}>
                <div id={`issue-${issue.id}`} className={cardStyles.metaLine}>
                  #{issue.number} •{" "}
                  <span
                    className={[
